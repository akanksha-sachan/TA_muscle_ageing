...
result_path <- "/ix/djishnu/Zarifeh/ML_MM/Aditi/result"
experiment <- "Aging"
...
```{r load}
# init a list of seurat objects for all samples
seurats <- list()
for (s1 in 1:length(sample_names)) {
  ...
  # add additional assay if present
  if (doCite) {
...
# QC for dead cells, low coverage cells, etc
...
# Merge all samples (4 mice) in one object (adding names for each sample in the meta-data)
```{r merge_objects}
...
# as samples were merges into one seurat object, they were kept in different layers of the RNA assay
...
# Finding differentially expressed features per cluster (cluster biomarkers)
...
# Reclustering CD4+ Tcells (reducing cluster numbers)
