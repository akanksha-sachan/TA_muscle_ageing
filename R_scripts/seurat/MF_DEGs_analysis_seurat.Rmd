...
female_h5ad <- MuDataSeurat::ReadH5AD("/ix/djishnu/Akanksha/datasets/ERCC1_KO_mice/objects/female_98_80_dense.h5ad")
male_h5ad <- MuDataSeurat::ReadH5AD("/ix/djishnu/Akanksha/datasets/ERCC1_KO_mice/objects/male_98_85_dense.h5ad")
# Save as RDS files
...
seurat_obj <- readRDS(file.path(out_path, "female_integrated.rds"))
# Subset to Skeleton MuSc cells only
seurat_stem <- subset(seurat_obj, subset = C_scANVI == "Skeleton MuSc")

# Check the subset
print(paste("Original cells:", ncol(seurat_obj)))
print(paste("Skeleton MuSc cells:", ncol(seurat_stem)))
print(table(seurat_stem$C_scANVI))
```
...
```
```{r plot umaps}
umap_sample <- DimPlot(seurat_stem, reduction = "umap", label = FALSE, label.size = 6, group.by = "condition", pt.size = 1) + 
               ggtitle("Female Skeleton MuSc UMAP")
ggsave(file.path(out_path, "/plots/female_musc_umap.png"), plot = umap_sample, width = 10.1, height = 10.1)
```
...
DEG_analysis <- function(gender_ko, gender_wt, cell_type, gender) {
  ...
  plot_png <- file.path(out_path, paste0("/plots/", gender, "_", cell_type, "_integrated_KO_DEGs_nmt.png"))
  ...
  # Find markers
  markers <- FindMarkers(seurat_stem, ident.1 = ko_ident, ident.2 = wt_ident, verbose = TRUE)
...
DEG_analysis("KO", "WT", "Skeleton MuSc", "Male")
```
